# --- START OF FILE assistant.py ---
# GÜNCEL VE TAM SÜRÜM (Girinti Hataları Giderildi + Samimi Cevap Modu)
...
class DesktopAssistant(QWidget):
    ...
    def mouseMoveEvent(self, event):
        if event.buttons() == Qt.LeftButton:
            ...
            # --- 2. KONUMLANDIRMA (MIKNATIS ETKİSİ) ---
            # Amaç: Resmin "el" kısmını fare imlecine denk getirmek.
            
            # Yükseklik Ayarı: Farenin ucu, resmin tepesinden 15 piksel aşağıda olsun (Eline denk gelsin)
            target_y = current_y - 15 

            if self.current_state == 'climb_right':
                # SAĞA GİDERKEN: El sağ üst köşede.
                # Pencerenin solunu, farenin soluna çekmeliyiz ki fare sağda kalsın.
                # Formül: Mouse X - Resim Genişliği + (Küçük bir pay)
                target_x = current_x - self.width() + 25 
            else:
                # SOLA GİDERKEN: El sol üst köşede.
                # Pencerenin solu, farenin olduğu yer olmalı.
                # Formül: Mouse X - (Küçük bir pay)
                target_x = current_x - 25

            # Pencereyi hesaplanan yeni konuma ışınla
            self.move(target_x, target_y)
            
            event.accept()

    def mouseReleaseEvent(self, event):
        # Fareyi bıraktığında düşme efekti ver
        if event.button() == Qt.LeftButton:
            self.set_state('fall')
            
            # 600ms sonra yere inme (land) efekti
            QTimer.singleShot(600, self.landing_animation)
            
    def landing_animation(self):
        # Eğer hala düşüyorsa (arada başka komut gelmediyse) land yap
        if self.current_state == 'fall':
            self.set_state('land')
            # 600ms sonra normale (idle) dön
